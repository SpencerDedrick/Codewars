...
// Filtering the original array into sub arrays
function plusMinus(arr) {
  //Determine % amount each value will hold based on number of elements in array
  //filter the array to only include numbers less than 0
  let less = arr.filter((x) => {
    return x < 0;
  });
  //filter the array to only include numbers greater than 0
  let more = arr.filter((x) => {
    return x > 0;
  });
  //filter the array to only include numbers equal to zero
  let zeroes = arr.filter((x) => {
    return x == 0;
  });
  //calculate what percent of the orignal array the sub array is
  let calcPercent = (nums) => {
    return (nums.length / arr.length).toFixed(6);
  };
  //log the results
  console.log(calcPercent(more));
  console.log(calcPercent(less));
  console.log(calcPercent(zeroes));
}
